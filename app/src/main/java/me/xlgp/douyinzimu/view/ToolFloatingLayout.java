...
public class ToolFloatingLayout extends BaseFloatingLayout {
    ...
    private void onViewListener() {
        ...
        //评论控制器
        ((SwitchMaterial) view.findViewById(R.id.pingLunSwitch)).setOnCheckedChangeListener((buttonView, isChecked) -> PingLun.getInstance().change(isChecked));
        view.findViewById(R.id.pinglunListBtn).setOnClickListener(v -> { //打开字幕列表layout
            if (ZWindowManager.getInstance().containView(zimuFloatinglayout)) {
